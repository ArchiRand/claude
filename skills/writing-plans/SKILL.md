...
Пиши исчерпывающие планы реализации, предполагая что исполнитель не имеет контекста кодовой базы. Документируй всё необходимое: какие файлы трогать в каждой задаче, код/конфиги, как верифицировать. Давай весь план мелкими задачами. DRY. YAGNI

Тесты — только если это принятая практика в проекте (проверь по `CLAUDE.md`, наличию тестовых директорий/раннера). Если в проекте тестов нет или их писать нецелесообразно — верификация задачи идёт через линтер/сборку/ручной прогон (curl, CLI, лог), а не через обязательный тест.
...
- «Внести изменение X в файл Y» — шаг
- «Запустить верификацию (команда/ручной прогон) и подтвердить ожидаемый результат» — шаг

Если в проекте приняты тесты — шаг с тестом добавляется как отдельная задача той же гранулярности, но это не обязательный элемент каждого плана.

## Шаблоны плана и задачи

Шаблон заголовка документа и структуры задачи — [references/plan-templates.md](references/plan-templates.md)
...
> 2. **Код-ревью изменений** — вызов скилла `vizor-code-review` по итоговому диффу.
>
...
3. Задача код-ревью:
   ...
   - **Если субагента нет** — задача должна явно содержать: «Вызови скилл `vizor-code-review` по изменениям, внесённым этим планом.»
